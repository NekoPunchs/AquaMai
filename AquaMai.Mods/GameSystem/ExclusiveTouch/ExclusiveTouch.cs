...
{
    public abstract class ExclusiveTouchBase(
        int playerNo,
        int vid,
        int pid,
        [CanBeNull] string serialNumber,
        [CanBeNull] string locationPath,
        byte configuration,
        int interfaceNumber,
        ReadEndpointID endpoint,
        int packetSize,
        int minX,
        int minY,
        int maxX,
        ...
        bool flip,
        int radius,
        float aExtraRadius = 0,
        float bExtraRadius = 0,
        float cExtraRadius = 0,
        float dExtraRadius = 0,
        float eExtraRadius = 0)
    {
        ...
        public void Start()
        {
            ...
            else
            {
                ...
                touchSensorMapper = new TouchSensorMapper(minX,
                                                          minY,
                                                          maxX,
                                                          maxY,
                                                          radius,
        ...
        private void ReadThread()
        {
            ...
            try
            {
                ...
                {
                    int bytesRead;
                    var ec = reader.Read(buffer, 100, out bytesRead); // 100ms 超时
        ...
        protected void HandleFinger(ushort x, ushort y, int fingerId, bool isPressed)
        {
            ...
            lock (touchLock)
            {
                ...
                if (isPressed)
                {
                    var touchMask = touchSensorMapper.ParseTouchPoint(x, y);
                    point.IsActive = true;
        ...
        {
            ulong mask = 0;
            for (var i = 0; i < allFingerPoints.Length; i++)
            {
