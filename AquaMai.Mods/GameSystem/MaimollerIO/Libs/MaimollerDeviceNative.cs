...
namespace AquaMai.Mods.GameSystem.MaimollerIO.Libs
{
    public class MaimollerDeviceNative : IMaimollerDevice
    {
        ...
        public void Open()
        {
            if (_hidThreadRunning)
            {
                throw new InvalidOperationException($"MaimollerDevice {_player + 1}P already opened");
            }
        ...
        public void Update()
        {
            if (!_hidThreadRunning)
            {
                throw new InvalidOperationException($"MaimollerDevice {_player + 1}P not opened");
            }
...
#region Input
        ...
        {
            if (buttonIndex1To8 < 1 || buttonIndex1To8 > 8)
            {
                return false;
            }
        ...
        public bool IsSystemButtonPressed(SystemButton button) => _inputLatch.ReadBit(SystemBitOffset + (int)button);

        public ulong GetTouchState() => _inputLatch.ReadBits(TouchMask);
...
#region Connection
        ...
        {
            var devices = HidDevices.Enumerate(VID, PID).Where(d => d.DevicePath.Contains("&mi_00#")).ToArray();
            if (devices.Length == 0)
            {
                return false;
            }
            ...
            foreach (var device in devices)
            {
                ...
                var devicePlayer = featureData[4] == 2 ? 1 : 0;
        ...
        private bool IsDeviceConnected() => _device != null && _connected;
