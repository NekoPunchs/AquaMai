...
namespace AquaMai.Core.Helpers
{
    public static class Shim
    {
        ...
        public static readonly string apiSuffix = Iife(() =>
        {
            try
            {
                var baseNetQueryConstructor = typeof(NetQuery<VOSerializer, VOSerializer>).GetConstructors().First();
                return ((INetQuery)baseNetQueryConstructor.Invoke([
                    .. baseNetQueryConstructor.GetParameters().Select((parameter, i) => i == 0 ? "" : parameter.DefaultValue)
                ])).Api;
            }
            catch (Exception e)
            {
        ...
        [HarmonyTranspiler, HarmonyPatch(typeof(NetHttpClient), "ReadCallback")]
        ...
        public static string RemoveApiSuffix(string api) => !string.IsNullOrEmpty(apiSuffix) && api.EndsWith(apiSuffix) ? api.Substring(0, api.Length - apiSuffix.Length) : api;
